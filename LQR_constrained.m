...
% (Q1,R1) should be very different from (Q2,R2)
...
%Q1 = gallery('randcorr',nx); R1 = gallery('randcorr',nu);
%Q2 = gallery('randcorr',nx); R2 = gallery('randcorr',nu);


% unconstrained solution, find upper bound for D0
[P,L,K] = dare(A,B,Q1,R1);
PK = iterate_calculate(Q2, Q2 + K'*R2*K, (A-B*K));
D0_upper = trace(PK)/3; % this is the upper bound. if D0 >= D0_upper, then it is as if no constraint
PK_J = iterate_calculate(Q1, Q1 + K'*R1*K, (A-B*K));
J0 = trace(PK_J)/3; % unconstrained minimum

% solve for D as objective, find lower bound for D0
[P,L,KD] = dare(A,B,Q2,R2);
PK = iterate_calculate(Q2, Q2 + KD'*R2*KD, (A-B*KD));
D0_lower = trace(PK)/3; % this is the lower bound. if D0 < D0_upper, then it is infeasible
...
while 1
    ...
    % sample x0_star ~ D and calculate
    IN = 20;
    ...
    % check if feasible
    % easy to check since quadratic term is Identity matrix
    ...
    if mod(t,200) == 0
        fprintf(' %d ',t/200);%fprintf(' elapesd time=%5.3f  \n',t2);
    end
...
obj_our = min( J_all(feasible) );
place_our = find( J_all_inf == min( J_all(feasible) ) );
place_our_almost = find( J_all_inf <= 1.0002 * min( J_all(feasible) ) , 1 );
obj_our_almost = J_all_inf(place_our_almost);
effective_time = time_our * place_our / t;
...
%{
% plot the constraint values in each itearation
plot(1:t-1, D_all,'LineWidth',1.8)
hold on;
line([1 t-1],[D0 D0],'LineWidth',2,'color','red','LineStyle', '--')
xlabel('iteration');ylabel('constraint value');
set(gca,'FontSize',15);
...
% plot the objective values in each itearation
plot(1:t-1, J_all,'LineWidth',2.8)
hold on;
line([1 t-1],[J0 J0],'LineWidth',2,'color','red','LineStyle', ':') % unconstrained minimum
xlabel('iteration');ylabel('objective value');
set(gca,'FontSize',15);
